    ...
    try {
        ...
        if (accounts.length === 0) {
            ...
            dashboardAccounts.innerHTML = accounts.slice(0, 4).map(account => {
                ...
                accountTypeElement.className = 'account-type';
                accountTypeElement.textContent = account.accountType;

                const accountStatusElement = document.createElement('span');
                accountStatusElement.className = `account-status ${account.active ? 'active' : 'inactive'}`;
                accountStatusElement.textContent = account.active ? 'Active' : 'Inactive';

                const accountBalanceLabelElement = document.createElement('div');
                accountBalanceLabelElement.className = 'account-balance-label';
                accountBalanceLabelElement.textContent = 'Available Balance';

                const accountBalanceAmountElement = document.createElement('div');
                accountBalanceAmountElement.className = 'account-balance-amount';
                accountBalanceAmountElement.textContent = formatCurrency(account.balance, account.currency);

                const accountIdElement = document.createElement('div');
                accountIdElement.className = 'account-id';
                const strongElement = document.createElement('strong');
                strongElement.textContent = 'Account:';
                accountIdElement.appendChild(strongElement);
                accountIdElement.appendChild(document.createTextNode(` ${formatAccountId(account.accountId)}`));

                const accountCard = document.createElement('div');
                accountCard.className = 'account-card';
                accountCard.appendChild(accountTypeElement);
                accountCard.appendChild(accountStatusElement);
                accountCard.appendChild(accountBalanceLabelElement);
                accountCard.appendChild(accountBalanceAmountElement);
                accountCard.appendChild(accountIdElement);

                return accountCard.outerHTML;
            }).join('');
        }
...
document.getElementById('transferForm').addEventListener('submit', async (e) => {
    ...
    showConfirmModal(
        'Confirm Transfer',
        `Transfer ${formatCurrency(amount)} from ${formatAccountId(fromAccountId)} to ${formatAccountId(toAccount
